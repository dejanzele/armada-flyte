...
async def test_create_submits_and_returns_handle(connector, mock_client):
    mock_client.submit_jobs.return_value = SimpleNamespace(
        job_response_items=[SimpleNamespace(job_id="01created", error="")]
    )

    # task_template.custom is empty here, so the connector falls back to its defaults.
    task_template = SimpleNamespace(custom=None)
...
async def test_create_raises_on_armada_error(connector, mock_client):
    mock_client.submit_jobs.return_value = SimpleNamespace(
